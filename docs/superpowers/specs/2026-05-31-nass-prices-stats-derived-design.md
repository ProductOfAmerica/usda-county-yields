...
**Status:** Design, revised three times against codex-spec (9, then 5, then 3 findings; P1 count 4 -> 2 -> 1, converging). Round 3 closed: the zero-shard sentinel invariant (a family that emits nothing must still write its audit or the bootstrap loop never clears), and all-artifacts-bump-version-together (no mixed v2/v3 tree). Round 3 confirmed the round-2 fixes (bootstrap inclusive-bound, candidate-counting canonical guard, consumer-first ordering) hold against the real code. Pending codex-spec round 4 for P1-clean confirmation, then user approval, then writing-plans.
