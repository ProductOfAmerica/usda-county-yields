...
**Status:** Design, revised twice. Round 1 closed 9 findings (consumer picker, validate-at-emit-site, canonical rule table, marketing-year mapping, derived-path, rollup shared-list, CV_% column, schema-history). Round 2 closed 5 more: bootstrap needed a real same-publication re-emit path (inclusive `discover` bound), not just a reorder; deploy must be strictly consumer-first, not simultaneous; the canonical guard must count rule-matching candidates, not post-mark flags; the phase-plan picker was aligned to the backward-compatible form; and schemas were clarified as static committed artifacts (not a sentinel gap). Pending codex-spec round 3, then user approval, then writing-plans.
